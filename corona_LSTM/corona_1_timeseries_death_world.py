...
death = (test_set.values).reshape(-1, 1)
death_scaled = sc.transform(death)
...
for i in range(timestamp, length):
    x_test.append(death_scaled[i-timestamp:i, 0])
...
'''
plt.plot(y_test, color = 'blue', label = 'Actual Death(s)')
plt.plot(predicted_value, color = 'red', label = 'Predicted Death(s)')
plt.title('Death Prediction - mul_layer_3 Model')
plt.xlabel('Time')
plt.ylabel('Total Death(s)')
plt.legend()
plt.show()
'''
...
ax.set_ylabel('Total Corona Death(s)', fontname="Palatino Linotype")
ax.set_title('Death Prediction - Vanilla Model',fontname="Palatino Linotype")
